...
class EnhancedDemo(Node):
    def __init__(self):
        super().__init__('enhanced_demo')
    ...
    def dispatch_task(self, target_place, description=""):
        request = ApiRequest()
        request.request_id = f'ENHANCED_{target_place}_{int(time.time())}'
...
def main():
    ...
    print("\n" + "="*70)
    print("   🏨 ENHANCED MULTI-FLOOR TRANSIT DEMO")
    print("      L1 → L3 → Navigate to Visible Location")
    print("="*70)
    ...
    # The L3_room1 waypoint should already be in the path from Phase 1
    # But let's explicitly wait for the robot to reach it
